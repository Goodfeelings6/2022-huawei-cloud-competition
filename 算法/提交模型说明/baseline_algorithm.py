...
def base_algorithm(data):
    ...
    def turnCount(start_box: dict, through_box: dict, end_box: dict, turn_config: dict, turn_ans: dict,lane_len:int,lane:str,video_id,car_track):
        """
        ...
        if lane_len==2: # 如果总车道数为2
            ...
            if lane=='1':  # 如果当前为第1车道，只可能左转或直行
                ...
                if  video_id=='video09' or video_id=='video10' or video_id=='video11' or video_id=='video12':
                    turn_ans['right']+=1
    ...
    def laneCount(start_box: dict, through_box: dict,end_box:dict, lane_ans: dict, lane_config: dict, lane_filter:int,video_id):
        """
        ...
        if X2-X1 != 0 and Y2-Y1!=0:
            ...
            for lane_id in lane_config:
                ...
                if lane_config[lane_id][0] <= x_value <= lane_config[lane_id][2] and Y3<lane_filter : 
                    #if video_id == 'video01':
    ...
    for filename, car_record in data.items():
        ...
        video_filter = count_config[video_id]['filter']
        video_filter_box = count_config[video_id]['filter_box']
        ...
        for car_id in car_routes.keys():
            # 过滤只出现一次的误识别框
            if len(car_routes[car_id]['track']) < video_filter_box:
                continue
            index=car_routes[car_id]['idx']

            # 从车辆轨迹中取出 起始点，过红灯停止线时点
            ...
            #if video_id == 'video05':
                #print(tt,{'car_start_box':car_start_box,'car_through_box':car_through_box,'car_end_box':car_end_box,'car_id':car_id},",")
                #tt+=1
                #print(car_id)
            # 判断车道
            lane_len,lane = laneCount(car_start_box, car_through_box,car_end_box,ans, video_lane, video_filter,video_id)
            # 判断转向
            turnCount(car_start_box, car_through_box, car_end_box, video_turn, ans, lane_len, lane,video_id,car_routes[car_id]['track'])
        return ans
